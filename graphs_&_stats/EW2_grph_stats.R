## This R script pulls in the output files for the Enigma project walk area two
## (EW2), and then merges in the bootsock Campylobacter presence data.  It then
...
## presence to integer
CampyData <- read.csv(file = "../EW2PosBootSock.csv",
...
LsData <- read.csv(file = '../EW2_livestock2.csv',
                   header = FALSE)
...
## create a column for number of negative bootsocks (neg.bs) per walk for odds
## ratio calculations
...
pdf(paste("EW2_plots_publish.pdf", sep=""))

## do all the plots for EW2 walk area using the input list and the
...
(EW2_all_rain_cor <- bind_rows(rain1d_pres_cor, rain2d_pres_cor, rain5d_pres_cor,
...
(EW2_all_temp_cor <- bind_rows(temp1d_pres_cor, temp2d_pres_cor, temp5d_pres_cor,
...
pdf("EW2_all_rain_cor.pdf", paper = "a4")
grid.table(EW2_all_rain_cor, theme = mytheme)
dev.off()

## print the pdf of all temp and Campylobacter presence correlations
pdf("EW2_all_temp_cor.pdf", paper = "a4")
grid.table(EW2_all_temp_cor, theme = mytheme)
dev.off()
...
pdf("EW2_qo_pres_cor.pdf", paper = "a4")
...
## print the pdf of overland flow and Campylobacter presence correlations
pdf("EW2_qs_pres_cor.pdf", paper = "a4")
...
pdf("EW2qt_pres_cor.pdf", paper = "a4")
